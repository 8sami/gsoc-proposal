...
* **Project Overview**:
    ...
    * Fetches live data instead of returning dummy data.
    ...
    * A few thoughts I had during the development of POC:
        ...
        * Will surely need a frontend implementation to be able to send notifications and alerts to patients and staff.
        * Since each encounter (visit) can have different medications and service requests etc, would it make more sense to prompt user to select an encounter when they message the plugin for, let's say, medications (if a patient has multiple encounters) or just list out medications from all encounters (as it currently does in the POC)?
        ...
        * What should happen if a staff member is also registered as a patient with the same phone number? I faced this scenario and wasn't sure how to handle it.
